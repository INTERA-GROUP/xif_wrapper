...
// Decoder: gets the issue_req and based on data on cvxif_instr_pkg provides issue_resp - combinational block
  ...
  //FIFO_commit signals
  assign fifo_commit_push = xif_issue_if.issue_valid && xif_issue_if.issue_ready; // If issue transaction then issue_req and issue_resp go to fifo, if not accepted or kill they will be discarded later on
